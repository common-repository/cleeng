...
jQuery(function() {
    ...
    public function parse_cleeng_items() {
        ...
        if (is_array($posts)) foreach ( $posts as $post ) {
            ...
            foreach ( $m[0] as $key => $content ) {
                ...
                $content = array(
                    'contentId' => $params['id'],
                    'shortDescription' => stripslashes(@$params['description']),
                    'price' => @$params['price'],
                    'itemType' => 'article',
                    'purchased' => false,
                    'shortUrl' => '',
        ...
        if ( $this->cleeng_preload_content ) {
            ...
            foreach ($rows as $cont) {
                ...
                if (in_array($cont['content_id'], $possiblyCached)) {
                    ...
                    $this->cleeng_content[$cont['content_id']]['itemType'] = $cont['item_type'];
    ...
    function get_layer_markup( $postId, $text, $content ) {
        ...
        echo "<script type='text/javascript'>var pickColor='".$options['button_color']."';
        jQuery(document).ready(function($)
        {
        $('.cleeng-button').css('border-radius','11px');
        $('.cleeng-button').css('background-color',pickColor);
        });
        </script>";
    ...
    ?>
        ...
        <p class="cleeng-prompt" style="<?php if ($purchased) {echo 'display:none;';} if($options['layout'] == 'button-only'){echo 'margin-bottom:0;';} ?>">
    ...
    ?>
        <?php if($options['layout'] == 'button-only'){echo ' style="width:267px;float:right;margin-top:10px;height:auto;"';}?>>
        <div class="cleeng-protected-content" id="cleeng-<?php echo $contentId ?>" rel="<?php echo $postId ?>"
        <?php if($options['layout'] == 'button-only'){echo ' style="display:none"';}?>> <?php _e('Exclusive content', 'cleeng'); ?></div>
        <div class="cleeng-layer-left" <?php if($options['layout'] == 'button-only'){echo ' style="display:none"';}?>></div>

        <div class="cleeng-text" <?php if($options['layout'] == 'button-only'){echo ' style="display:none"';}?>>
            <?php /*
        ...
        if ( $auth ) {
            echo ' style="display:none"';
                ...
                <a class="cleeng-hlink cleeng-login" href="javascript:"><?php _e('Log-in', 'cleeng'); ?></a>
            </div>
    ...
    ?>>
        ...
        <div class="cleeng-text-bottom" <?php if($options['layout'] == 'button-only'){echo "style='height:82px'";}?>>
            <div class="cleeng-textBottom">
                <div class="cleeng-purchaseInfo-grad" <?php if($options['layout'] == 'button-only'){echo ' style="display:none"';}?>>
                </div>
                <div class="cleeng-purchaseInfo" <?php if($options['layout'] == 'button-only'){echo ' style="background:none"';}?>>
                    <div class="cleeng-purchaseInfo-text">
                        ...
                        <?php if ($options['payment_method'] == 'cleeng-only' || $price < 0.49) : ?>
                                ...
                                ?>
<?php if($options['pay_per_item'] ==true) {?>
...
<?php }?>
                ...
                <div class="cleeng-whatsCleeng" <?php if($options['layout'] == 'button-only'){echo ' style="display:none"';} else {
                    echo ' style="background-image: url(' . $cleeng->getLogoUrl( $contentId, 'cleeng-small' ) . ')"';
                }?>
                    >
                      <a href="<?php echo $cleeng->getUrl() ?>/what-is-cleeng">Powered by</a>
                </div>
            </div>
        </div>

        <div class="cleeng-layer-right" <?php if($options['layout'] == 'button-only'){echo ' style="display:none"';}?>></div>
    </div>
