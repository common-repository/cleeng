...
var CleengWidget = {
    ...
    sliderToPrice: [
        0, 0.49, 0.54, 0.59, 0.69, 0.79, 0.89, 0.99,
    ...
    init: function(){
		...
		jQuery("#preview_form").submit();           //save settings after registration
            });
            return false;
        });
    ...
    showContentForm: function(content) {
        ...
        content.shortDescription = typeof content.shortDescription === 'undefined' ? '' :
    ...
    getStartMarker: function(content) {
        ...
        startMarker += ' description="' + content.shortDescription.replace(/\\/g,'\\\\').replace(/"/g, '\\"') + '"';
    ...
    getContentFromEditor: function(contentId) {        
        ...
        if (ct && ct[0]) {
            ...
            if (id && id[1]) {
                ...
                if (content.contentId.indexOf("t")=='-1') {
                    var description = opening[0].match(/description="(.*?[^\\]{1})"/i);
                } else {
                    var description = opening[0].match(/description="((Q.|[^Q])*)"/i);
                }
